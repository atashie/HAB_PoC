...
|------|--------|-------|--------|--------------|
| 1 · ingest | `01_ingest.py` | CyAN cache, FL lakes | — | Verify the two inputs are present (download the full set from NASA Earthdata if the cache is empty); flag any gaps in the weekly series. |
...
Regenerated by `04_evaluate.py` (`outputs/eval_metrics.md`); base rate ≈ 0.267.

| h | lean AUC | persist AUC | clim AUC | lean onset-AUC | clim onset-AUC |
|--:|--|--:|--:|--|--:|
| 0 | 0.980 [0.973, 0.987] | 0.946 | 0.941 | 0.836 [0.781, 0.881] | 0.851 |
| 1 | 0.968 [0.958, 0.977] | 0.926 | 0.941 | 0.804 [0.756, 0.847] | 0.863 |
| 2 | 0.957 [0.943, 0.969] | 0.912 | 0.941 | 0.774 [0.717, 0.821] | 0.866 |
| 3 | 0.949 [0.933, 0.961] | 0.899 | 0.941 | 0.765 [0.712, 0.809] | 0.875 |
| 4 | 0.942 [0.925, 0.957] | 0.892 | 0.941 | 0.752 [0.700, 0.797] | 0.884 |

**Reading it honestly.** All-sample AUC is high but **autocorrelation-dominated** — persistence
scores nearly as well, so it is not the result. The decision-relevant signal is **onset-AUC**
(currently-clear lakes, where persistence has no skill by construction). The lean model has
genuine onset skill, but a per-lake seasonal **climatology — itself only a baseline — is
competitive-to-better** on the onset alert. So this 2-feature model's value is **simplicity +
explainability while staying competitive**, not beating every baseline; the richer real-time-CyAN
ladder in [`models/`](models/RESULTS-SUMMARY.md) is what pushes onset skill higher. The label is a
satellite realization, not toxin. Correlation, not causation.
...
Deterministic (seed 42); runs against the CyAN rasters already cached in the repo. See
